...
const verifyUserType = async () => {
    ...
    if(json.response === true && json.type === 'caregiver'){
...
const requestPenddings = async () => {
    const response = await fetch('http://localhost:5000/penddings/caregiver', {
        method: 'GET',
        headers: {
...
const renderPenddings = async (json, association) => {
    ...
    for(let i in json){
        ...
        if(json[i].inAnalysis === true){
            const areaElement = card.querySelector('.buttonsCard');
            areaElement.classList.add('resultRequest')
            areaElement.innerHTML = 'Aguardando confirmação!';
        }else{
...
const recused = (e) => {
    ...
    if(indentifier === null || indentifier === undefined){
        return;
    }
...
const requestOption = async (option, indentifier) => {
    ...
    const response = await fetch('http://localhost:5000/send/association/responsible', {
        method: 'POST',
        headers: {
            'Authorization': `bearer ${localStorage.getItem('token')}`,
            'Content-type': 'application/json'
